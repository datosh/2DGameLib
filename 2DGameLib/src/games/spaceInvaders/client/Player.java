...
public class Player {
	...
	final private static int SHOOTINGDELAY = 500_000_000; //time in nanoseconds
	...
	Player(int windowWidth, int windowHeight){
		try {
			image = ImageIO.read(new File("images/space_invaders/player.png")).getScaledInstance(SCALED_X, SCLAED_Y, Image.SCALE_SMOOTH);			
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void move(int deltaX, int deltaY){
		...
		if(positionX < 0) {
			positionX = 0; 
		} else if(positionX + SCALED_X > windowWidth) {
			positionX = windowWidth - SCALED_X;
		}
		collider.setPosition(positionX + 4, positionY + SCLAED_Y / 3 * 2);
	}
}
